...
public class Blocks implements Comparable<Blocks>
{
	...
	public static HashMap<String, Integer> getUniqCount(int min, int max) throws FileNotFoundException, IOException {
		...
		for (int i = 0; i < blocks.size(); ++i)
		{
			...
			if(b.getNumber()>=min && b.getNumber()<=max) {
		...
		for(int i = 0; i < uniqMiners.size(); i++) {
			freq.put(uniqMiners.get(i), uniqMinersFreq.get(i));
		}
	...
	public static int[] getTimeDiff(Blocks first, Blocks second) {
		int diffInSeconds = (int) Math.abs(first.timestamp - second.timestamp);
		int diffInMinutes = diffInSeconds / 60;
		int diffInHours = diffInMinutes / 60;
		diffInSeconds = diffInSeconds % 60;
		...
		return new int[] {diffInHours, diffInMinutes, diffInSeconds};
	}
